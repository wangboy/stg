// 程序入口
class GameMain {
    private pro: Laya.Label = new Laya.Label();


    public static hero: Hero;           //自己
    public static enemy: Laya.Sprite;   //敌人列表
    public static ufo: Laya.Sprite;     //UFO列表
    public static gameInfo: GameInfo;   //游戏面板
    ...
    static createEnemy(): void {
        if (GameMain.difficulty < 0.2)
            GameMain.difficulty += 0.001;

        console.log("难度系数:" + this.difficulty);
        var type: number = Math.random() < (0.75 - GameMain.difficulty) ? 1 : Math.random() < (0.9 - GameMain.difficulty) ? 2 : 3;

        var enemy: Enemy = Laya.Pool.getItemByClass("enemy", Enemy);
